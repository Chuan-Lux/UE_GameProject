		...
		if (bFoundPlatform)
		{
			for (auto& Platform : Platforms)
			{
				...
				Futures.Add(WwiseAsync([this, Platform, PlatformPath] {
					...
					if (!IWwiseDirectoryVisitor<FPlatformRootDirectoryVisitor>::IterateDirectory(RootVisitor.DirectoryVisitor, PlatformPath) ||
...
bool WwiseDirectoryVisitor::FPlatformRootDirectoryVisitor::StartJobIfValid()
{
	...
	if (!PlatformFiles.MediaDirectory.IsEmpty())
	{
		...
		Futures.Add(WwiseAsync([this, Elem] {
			...
			IWwiseDirectoryVisitor<FMediaVisitor>::IterateDirectory(MediaVisitor.DirectoryVisitor, Elem);
	...
	for (const auto& Elem : PlatformFiles.AutoSoundBankDirectories)
	{
		Futures.Add(WwiseAsync([this, Elem] {
			...
			IWwiseDirectoryVisitor<FSoundBankVisitor>::IterateDirectory(SoundBankVisitor.DirectoryVisitor, Elem);
			auto BaseVisitor = IWwiseDirectoryVisitor<WwisePlatformVisitor>(
